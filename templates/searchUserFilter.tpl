...
<script type="text/javascript">
	...
	$('select[id^="filterUserGroupId"]').change(function(){
		$('#{$formId}').submit();
	}); 
</script>
...
<form class="pkp_form filter" id="{$formId}" action="{url op="fetchGrid"}" method="post">
	...
	{fbvFormArea id="userSearchFormArea"|concat:$filterData.gridId}
		...
		{fbvFormSection}
			{fbvElement type="select" onchage="" name="filterUserGroupId" id="filterUserGroupId"|concat:$filterData.gridId from=$filterData.userGroupOptions selected=$filterSelectionData.filterUserGroupId size=$fbvStyles.size.SMALL translate=false inline="true"}
			...
			{fbvFormButtons hideCancel=true submitText="common.search" inline="true"}
		{/fbvFormSection}
